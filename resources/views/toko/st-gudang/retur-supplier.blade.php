                ...
                <div class="col-xl-12 col-lg-12 col-sm-12  layout-spacing">
                    <div class="widget-content widget-content-area br-6">
                                    ...
                                    <div class="modal fade" id="add" tabindex="-1" role="dialog"
                                        ...
                                        <div class="modal-dialog modal-xl modal-dialog-centered" role="document">
                                            <div class="modal-content">
                                                ...
                                                <form id="addForm"
                                                    action="{{ route('store-retursupplier', ['slug' => $toko->slug]) }}"
                                                    ...
                                                    <div class="modal-body">
                                                            ...
                                                            <div class="col-12">
                                                                ...
                                                                <div class="form-group mb-4">
                                                                    <textarea class="form-control" name="catatan" value="{{ old('catatan') }}" rows="4"
                                                                        placeholder="Catatan Retur Supplier"></textarea>
                                                                </div>
                                                            </div>
                                                            <div class="col-12">
                                                                <div class="custom-file-container"
                                                                    data-upload-id="mySecondImage">
                                                                    <label>Foto Bukti Retur Supplier (bisa lebih dari 1) <a
                                                        ...
                                                        <div class="row align-items-center">
                                                            ...
                                                            <div class="col-lg-4 d-flex justify-content-end align-items-center"
                                                                style="margin-left: auto" id="restockSelect">
                                                                <select class="selectpicker form-control"
                                                                    data-live-search="true" name="restock">
                                                                    <option value="" selected disabled>Pilih No
                                                                        Series Restock</option>
                                                                    @foreach ($restock as $rst)
                                                                        <option value="{{ $rst->noseries }}">
                                                                            {{ $rst->noseries }}</option>
                                                                    @endforeach
                                                                </select>
                                                            </div>
                        ...
                        <table id="html5-extension" class="display table table-hover non-hover table-striped"
                            ...
                            <tbody>
                                @foreach ($retursupplier as $rs)
                                    <tr>
                                        <td>{{ $rs->noseries }}</td>
                                        <td>{{ ucwords($rs->user->name) }}</td>
                                        <td>{{ \Carbon\Carbon::parse($rs->tgl_retur)->translatedFormat('l, d F Y H:i') }}
                                        </td>
                                        <td>{{ $rs->restock->noseries }}</td>
                                        <td class="text-center">
                                            <button type="button" data-toggle="modal"
                                                data-target="#lihatbukti-{{ $rs->id }}"
                                                data-image="{{ json_encode($rs->gambar->map(function ($gb) {return asset('storage/' . $gb->path);})->toArray()) }}"
                                                title="Lihat Bukti">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                                    viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                        ...
                                        <!-- Modal Detail -->
                                        <div class="modal fade" id="tabsModal-{{ $rs->id }}" tabindex="-1"
                                                    ...
                                                    <div class="modal-body">
                                                        <ul class="nav nav-tabs mb-3" id="myTab" role="tablist">
                                                            <li class="nav-item">
                                                                <a class="nav-link active"
                                                                    id="contact-tab-{{ $rs->id }}"
                                                                    data-toggle="tab" href="#contact-{{ $rs->id }}"
                                                                    role="tab" aria-controls="contact"
                                                                    aria-selected="false">Catatan</a>
                                                            </li>
                                                            <li class="nav-item">
                                                                <a class="nav-link" id="staff-tab-{{ $rs->id }}"
                                                                    data-toggle="tab" href="#staff-{{ $rs->id }}"
                                                        ...
                                                        <div class="tab-content" id="myTabContent">
                                                            <div class="tab-pane fade show active"
                                                                id="contact-{{ $rs->id }}" role="tabpanel"
                                                                aria-labelledby="contact-tab-{{ $rs->id }}">
                                                                <p class="modal-text">{{ $rs->catatan ?? '-' }}</p>
                                                            </div>
                                                            <div class="tab-pane fade" id="staff-{{ $rs->id }}"
                                                                role="tabpanel"
                                                                aria-labelledby="staff-tab-{{ $rs->id }}">
                                                                <div class="product-list">
                                                                    @foreach ($rs->detailretursupplier as $index => $detail)
...
@section('js-custom')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#restockSelect select').on('change', function() {
                ...
                if (selectedValue && selectedValue !== '') {
                    $.ajax({
                        url: '{{ route('api.restock-detail', ':no_series') }}'.replace(
                        ...
                        success: function(response) {
                            if (response.success) {
                                ...
                                data.forEach(item => {
                                    const $template = $(restockTemplate);
                                ...
                                $listContainer.append(
                                    '<div class="empty-text" style="text-align: center; padding: 20px; color: #b0131e; font-weight: 900; font-size: 17.5px;">Data restock tidak ditemukan!</div>'
                                );
                            }
                        },
                        error: function(xhr) {
                    ...
                    $listContainer.append(
                        '<div class="empty-text" style="text-align: center; padding: 20px; color: #b0131e; font-weight: 900; font-size: 17.5px;">Silahkan pilih no series restock terlebih dahulu!</div>'
                    );
            ...
            $('#add').on('shown.bs.modal', function() {
                $('#restockSelect select').selectpicker({
                    liveSearch: true
                }).selectpicker('refresh');
                const $select = $('#restockSelect select');
            ...
            $('#addForm').on('submit', function(e) {
                ...
                if ($select.val() === '' || $select.val() === null || $listContainer.children().length ===
                    ...
                    e.preventDefault();
                    Snackbar.show({
                        text: 'Silakan pilih No Series restock sebelum simpan!',
                        pos: 'bottom-left'
                    });
                    return;
                }
